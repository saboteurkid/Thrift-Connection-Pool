...
public class PoolWatchThread<T extends TServiceClient> implements Runnable {
     ...
     * 连接池对象
     */
    private final ThriftConnectionPool<T> thriftConnectionPool;

    /**
     * 连接分区对象
     */
    private final ThriftConnectionPartition<T> thriftConnectionPartition;
     ...
     * 最大连接获取失效次数
     */
    private final int maxConnectionCreateFailedCount;
            ...
            try {
                ...
                if (maxNewConnections > 0 && !this.thriftConnectionPool.poolShuttingDown) {
                    ...
                    if (this.thriftConnectionPartition.getCreatedConnections() < this.thriftConnectionPartition
                            .getMinConnections()) {
                        System.out.println("Do fill connection");
     ...
     * 新的连接会添加到分区中
     *
     * @param connectionsToCreate 需要创建的连接数
     * @throws InterruptedException 当添加操作被中断的时候抛出该异常
     */
    private void fillConnections(int connectionsToCreate) throws InterruptedException {
        try {
            for (int i = 0; i < connectionsToCreate; i++) {
                ...
                System.out.println("Add connection to partition.");
