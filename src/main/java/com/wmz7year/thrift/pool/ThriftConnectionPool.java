...
public class ThriftConnectionPool<T extends TServiceClient> implements Serializable, Closeable {
     ...
     * @throws ThriftConnectionPoolException 当发生错误的时候抛出该异常信息
     */
     ...
     * 添加新的thrift服务器的方法
     *
     * @param thriftServerInfo 需要添加的服务器信息
     * @return true为添加成功 false为添加失败
     * @throws ThriftConnectionPoolException 当添加thrift服务器出现问题时抛出该异常
     */
     ...
     *
     * @param thriftServerInfo 需要删除的服务器信息
     ...
     * 根据配置信息对象创建连接分区的方法
     *
     * @param thriftServerInfo 配置信息对象
     * @return 连接分区对象
     * @throws ThriftConnectionPoolException 创建连接分区过程中可能产生的异常
     */
     ...
     * 根据配置获取原始连接的方法
     *
     * @param serverInfo thrift服务器信息
     * @return thrift客户端连接对象
     * @throws ThriftConnectionPoolException 当获取连接出现问题时抛出该异常
     */
    private ThriftConnection<T> obtainRawInternalConnection(ThriftServerInfo serverInfo)
        ...
        if (this.thriftServiceType == ThriftServiceType.SINGLE_INTERFACE) {
            connection = new DefaultThriftConnection<>(serverInfo.getHost(), serverInfo.getPort(),
                    this.connectionTimeOut, this.config.getThriftProtocol(), this.config.getClientClass(), config.getTransportProvider());
        } else {
            connection = new MulitServiceThriftConnecion<>(serverInfo.getHost(), serverInfo.getPort(),
                    this.connectionTimeOut, this.config.getThriftProtocol(), this.config.getThriftClientClasses(), config.getTransportProvider());
        }
        return connection;
    }
     ...
     *
     * @return 连接对象
     * @throws ThriftConnectionPoolException 当获取连接出现错误时抛出该异常
     */
     ...
     * 获取一个thrift原始连接对象的方法
     *
     * @param thriftConnectionHandle thrift连接代理类对象
     * @return thrift连接对象
     * @throws ThriftConnectionPoolException 当获取连接出现问题时抛出该异常
     */
     ...
     *
     * @param connection 需要回收的连接对象
     * @throws ThriftConnectionPoolException 回收过程中可能产生的异常
     */
     ...
     *
     * @param connectionHandle 连接代理对象
     * @throws ThriftConnectionPoolException 回收连接过程出现错误时抛出该异常
     */
     ...
     *
     * @param connectionPartition 需要检测的连接分区
     */
